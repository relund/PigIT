...
public:
    ...
    void CloseWriter() {
      ...
      log << "Create HMDP ...\n\n";
   	log << "  Statistics:\n";
		log << "    states : " << sTotal << "\n";
		log << "    actions: " << aTotal << "\n";
		log << "    weights: " << wLblLth << " with checksums " << vec2String<flt>(wCheck) << "\n\n";
    ...
    void Action(const vector<int> &scope, const vector<int> &index,
        ...
        for (int i=0; i<wCheck.size(); ++i) wCheck[i] += weights[i];
...
private:
   ...
   TimeMan cpuTime;
   vector<flt> wCheck;   ///< Checksum of all weights  
};
